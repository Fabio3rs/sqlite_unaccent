...
TEST_F(SqliteMem, Unaccent_Works) {
    auto res = scalarText("SELECT UNACCENT('ÁRVORE, Açúcar e Ímã');");
    EXPECT_EQ(res, "ARVORE, Acucar e Ima");
}

TEST_F(SqliteMem, Unaccent_DottedCapitalI) {
    auto res = scalarText("SELECT UNACCENT('İstanbul');");
    EXPECT_EQ(res, "Istanbul");
}

TEST_F(SqliteMem, Unaccent_IsCasePreserving) {
    ASSERT_NO_FATAL_FAILURE(scalarText("CREATE TABLE t(x TEXT);"));
    ASSERT_NO_FATAL_FAILURE(
        scalarText("INSERT INTO t VALUES('ÁRVORE'),('árvore');"));
    auto res = scalarText("SELECT group_concat(UNACCENT(x), '|') "
                          "FROM t ORDER BY x;");
    EXPECT_EQ(res, "ARVORE|arvore");
}
...
TEST_F(SqliteMem, IndexOnExpression_Demo) {
    ...
    ASSERT_NO_FATAL_FAILURE(scalarText("CREATE TABLE t(x TEXT);"));
    ASSERT_NO_FATAL_FAILURE(
        scalarText("CREATE INDEX idx ON t(UNACCENT_LOWER(x));"));
    ASSERT_NO_FATAL_FAILURE(
        scalarText("INSERT INTO t VALUES('Coração'),('coracao'),('CORACAO');"));
    auto res = scalarText(
